...
We’ve designed a multi-component system for trustlessly linking together Pastel Network and Solana in such a way that regular Solana users will be able to take advantage of services offered by Pastel (such as file storage with Cascade, near-duplicate image detection and fingerprinting with Sense, inference requests across many AI modalities with Inference Layer, etc.) all by interacting with a Solana native program, and paying for services using SOL natively. Importantly, this is achieved in a completely decentralized and trustless way, without requiring any centralized server to handle requests or to link together the Pastel and Solana blockchains. Critically, all this is done without exposing Solana’s end users to any economic or “credit risk” (i.e. a scenario in which they paid SOL for services that are then never rendered on Pastel).
